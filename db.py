...
Connects to SQL Server LocalDB: (localdb)\\MSSQLLocalDB → SmartSpendDB
"""
...
# Fallback driver name (some installs use version 18)
_CONN_STR_FALLBACK = (
...
def get_connection():
    """Return an open pyodbc connection. Tries driver 17 then 18."""
    try:
...
def is_db_available() -> bool:
    """Quick health-check; returns False if SQL Server LocalDB is unreachable."""
    try:
        conn = get_connection()
        conn.close()
        return True
...
def save_expense(
    ...
    """
    sql = """
        INSERT INTO dbo.ExpenseReports (
...
def update_report_status(report_id: int, status: str, reviewed_by: str) -> None:
    ...
    sql = """
        ...
        SET Exp_Status  = ?,
            Reviewed_By = ?,
            Review_Date = CAST(GETDATE() AS DATE)
        WHERE Report_ID = ?
    """
    with get_connection() as conn:
        cursor = conn.cursor()
...
def manager_approve(report_id: int, manager_name: str) -> None:
    ...
    sql = """
        ...
        SET Exp_Status           = 'Approved by Manager',
            Manager_Name         = ?,
            Manager_Decision_Date = CAST(GETDATE() AS DATE)
        WHERE Report_ID = ?
    """
    with get_connection() as conn:
        cursor = conn.cursor()
...
def reimburse_report(report_id: int, fo_name: str) -> None:
    ...
    sql = """
        ...
        SET Exp_Status     = 'Reimbursed',
            Reimbursed_By  = ?,
            Reimbursed_Date = CAST(GETDATE() AS DATE)
        WHERE Report_ID = ?
    """
    with get_connection() as conn:
        cursor = conn.cursor()
...
def get_all_reports(user_id: int | None = None) -> pd.DataFrame:
    ...
    (used to scope Employee view to their own submissions).
    """
    if user_id is not None:
        sql = "SELECT * FROM dbo.vw_ExpenseReports WHERE Submitted_By = ? ORDER BY Created_At DESC"
        with get_connection() as conn:
            return pd.read_sql(sql, conn, params=[user_id])
    sql = "SELECT * FROM dbo.vw_ExpenseReports ORDER BY Created_At DESC"
    with get_connection() as conn:
...
def get_report_by_id(report_id: int) -> pd.Series | None:
    ...
    sql = "SELECT * FROM dbo.vw_ExpenseReports WHERE Report_ID = ?"
    with get_connection() as conn:
...
def get_dept_spend(dept_id: int) -> float:
    ...
    sql = """
        SELECT ISNULL(SUM(Exp_TotalAmount), 0)
        FROM dbo.ExpenseReports
        WHERE Dept_ID = ?
          AND Exp_Status IN ('Pending Review', 'Approved')
    """
    with get_connection() as conn:
        cursor = conn.cursor()
...
def get_dept_budget(dept_id: int) -> float:
    ...
    sql = "SELECT Budget FROM dbo.Departments WHERE Dept_ID = ?"
    with get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(sql, dept_id)
        row = cursor.fetchone()
...
def get_manager_queue() -> pd.DataFrame:
    ...
    sql = """
        SELECT * FROM dbo.vw_ExpenseReports
...
def resubmit_report(report_id: int) -> None:
    """
    Reset a rejected report back to 'Pending Manager Review' so the
    employee can have it re-evaluated after corrections.
    Clears manager decision fields.
    """
    sql = """
        UPDATE dbo.ExpenseReports
...
def get_submission_trend() -> pd.DataFrame:
    ...
    sql = """
        SELECT
            CAST(DATEADD(DAY, 1 - DATEPART(WEEKDAY, Submission_Date), Submission_Date) AS DATE)
                AS Week_Start,
            COUNT(*)               AS Submissions,
            AVG(ISNULL(AI_Risk_Score, 0)) AS Avg_Risk,
            SUM(Exp_TotalAmount)   AS Total_Amount,
            SUM(CASE WHEN Triage_Level = 'AUDIT FLAG'    THEN 1 ELSE 0 END) AS Audit_Flags,
            SUM(CASE WHEN Triage_Level = 'STANDARD REVIEW' THEN 1 ELSE 0 END) AS Standard
        FROM dbo.ExpenseReports
        WHERE Submission_Date >= DATEADD(DAY, -90, GETDATE())
        GROUP BY DATEADD(DAY, 1 - DATEPART(WEEKDAY, Submission_Date), Submission_Date)
        ORDER BY Week_Start
    """
    with get_connection() as conn:
        return pd.read_sql(sql, conn)
...
def get_dept_spend_summary() -> pd.DataFrame:
    ...
    sql = """
        SELECT
            ...
            d.Dept_Name,
            d.Budget,
            ISNULL(SUM(e.Exp_TotalAmount), 0) AS Current_Spend,
            ISNULL(COUNT(e.Report_ID), 0)      AS Report_Count,
            ISNULL(AVG(e.AI_Risk_Score), 0)    AS Avg_Risk
        FROM dbo.Departments d
        LEFT JOIN dbo.ExpenseReports e
...
def hash_password(password: str) -> str:
    ...
    Returns a hex string in the format  salt_hex:key_hex.
    """
    salt = os.urandom(16)
...
def verify_password(password: str, stored_hash: str) -> bool:
    """Verify a plain-text password against a stored hash from hash_password()."""
    try:
...
def get_user(username: str) -> dict | None:
    """Return user record dict for the given username, or None if not found."""
    sql = """
...
def update_report(report_id: int, updates: dict) -> None:
    ...
    Allowed keys: Report_Title, Seller, Payment_Method, Description,
    ...
    sql = f"UPDATE dbo.ExpenseReports SET {set_clause} WHERE Report_ID = ?"
    with get_connection() as conn:
        cursor = conn.cursor()
...
def delete_reports(report_ids: list) -> int:
    """
    Hard-delete one or more expense reports by Report_ID.
    Returns the number of rows deleted.
    """
    if not report_ids:
        return 0
    placeholders = ", ".join("?" * len(report_ids))
    sql = f"DELETE FROM dbo.ExpenseReports WHERE Report_ID IN ({placeholders})"
    with get_connection() as conn:
        cursor = conn.cursor()
